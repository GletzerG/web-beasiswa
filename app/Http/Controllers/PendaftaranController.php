...
class PendaftaranController extends Controller
{
    ...
    public function store(Request $request, Beasiswa $beasiswa)
    {
        ...
        // Upload files
        $transkripName = time() . 'transkrip' . $request->file('file_transkrip')->getClientOriginalName();
        $ktpName = time() . 'ktp' . $request->file('file_ktp')->getClientOriginalName();
        $kkName = time() . 'kk' . $request->file('file_kk')->getClientOriginalName();
